...
test('test compactRange() frees disk space after key deletion', function (t) {
  var key1 = Buffer.from('000000');
  var key2 = Buffer.from('000001');
  ...
  db.put(key1, val1, function() {
    db.put(key2, val2, function() {
      db.compactRange(key1, key2, function() {
        db.approximateSize(Buffer.from('0'), Buffer.from('z'), function(err, sizeAfterPuts) {
          db.del(key1, function() {
            db.del(key2, function() {
              db.compactRange(key1, key2, function() {
                db.approximateSize(Buffer.from('0'), Buffer.from('z'), function(err, sizeAfterCompact) {
